    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Gerador e checker de nitro")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Gerador e checker de nitro\a',
        ...
        if USE_WEBHOOK:
            ...
            self.slowType(
                "", .02, newLine=False)
            url = 'https://discordapp.com/api/webhooks/980269427457163314/b-kBAUK4P6hQq0g8pCDfM7f-Uhus5OQe-wo8AIicqGgJ9ogGElk2g5k0Vc2bT14089lX'  # Get the awnser
            ...
            if webhook is not None:
                DiscordWebhook(  # Let the user know it has started logging the ids
                        url=url,
                        content=f"```+1 nitro sendo gerado agora! ```"
                    ).execute()
        ...
        for s in c:  # Loop over the amount of codes to check
            ...
            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Gerador & Checker Beta - {len(valid)} Valido | {invalid} Invalido ")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Gerador & Checker Beta - {len(valid)} Valido | {invalid} Invalido \a', end='', flush=True)
